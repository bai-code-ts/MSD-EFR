...
class EncoderLayer_c3(nn.Module):
    ...
    def forward(self, x, attn_mask1=None, attn_mask2=None, tau=None, delta=None):

        # print(x.shape)    # torch.Size([32, 28, 256])
        B,N,D = x.size()
        C = int(N//self.k)
        x0 = x.reshape(-1, C, D)
        # print('x0',x0.shape, attn_mask1.shape, attn_mask2.shape)      # x0 torch.Size([128, 7, 256]) torch.Size([128, 1, 7, 7])
        x1, attn = self.attention1(
            x0, x0, x0,
        ...
        x2 = x.reshape(-1, self.k, D)
        x3, attn = self.attention2(
            x2, x2, x2,
...
class Encoder_c3(nn.Module):
    def __init__(self, attn_layers, conv_layers=None, norm_layer=None):
        super(Encoder_c3, self).__init__()
        self.attn_layers = nn.ModuleList(attn_layers)
        self.conv_layers = nn.ModuleList(conv_layers) if conv_layers is not None else None
        self.norm = norm_layer
    ...
    def forward(self, x, attn_mask1=None,attn_mask2=None, tau=None, delta=None):
        ...
        if self.conv_layers is not None:
            for i, (attn_layer, conv_layer) in enumerate(zip(self.attn_layers, self.conv_layers)):
                delta = delta if i == 0 else None
                x, attn = attn_layer(x, attn_mask1=attn_mask1, attn_mask2=attn_mask2, tau=tau, delta=delta)
                x = conv_layer(x)
                attns.append(attn)
            x, attn = self.attn_layers[-1](x, tau=tau, delta=None)
            attns.append(attn)
