...
class RollingForecast(ForecastingStrategy):
    ...
    def _eval_batch(
        ...
        print(len(targets),"\t",len(all_predicts))
        all_batch_data = {}
        ...
        # ##################################################################
        # # 可视化每个批次的预测与真实值
        # for i, (pred, target) in enumerate(zip(all_predicts, targets)):
        #     pred_values = pred[:, 0]  # 预测值
        #     true_values = target[:, 0]  # 真实值
        # for i, (pred, target) in enumerate(zip(all_predicts, targets)):
        #     # 选择第一个维度作为展示
        #     pred_values = pred[:, 0]  # 预测值
        #     true_values = target[:, 0]  # 真实值
        ...
        #     plt.plot(pred_values, label='Predicted Values', linestyle='--', linewidth=2)
        #     plt.legend()
        #     plt.title(f'Batch {i} - Prediction vs True')
        #     plt.xlabel('Time Step')
        #     plt.ylabel('Value')
        #     plt.tight_layout()
        #
        #     # 保存图像（每个批次一个图）
        #     folder_path = './results/rolling_predictions/'
        #     if not os.path.exists(folder_path):
        #         os.makedirs(folder_path)
        #
        #     plt.savefig(os.path.join(folder_path, f'batch_{i}_prediction_vs_true.png'), dpi=300)
        #     plt.close()
        #
        #     # 保存当前批次的预测值和真实值到字典
        #     all_batch_data[f'batch_{i}'] = {
        ...
        # 假设
        # all_predicts.shape = (num_batches, horizon, num_channels)
        # targets.shape = (num_batches, horizon, num_channels)
        num_batches, horizon, num_channels = all_predicts.shape
        step_size = 1  # 滚动步长，按实际情况设置

        # 计算总时间步
        total_time = step_size * (num_batches - 1) + horizon

        # 每个通道创建数组存放最终预测和真实值
        final_pred = np.zeros((total_time, num_channels))
        final_true = np.zeros((total_time, num_channels))

        # 合并每个批次的预测结果，覆盖重复时间步
        for batch_id in range(num_batches):
            start_idx = batch_id * step_size
            end_idx = start_idx + horizon
            final_pred[start_idx:end_idx, :] = all_predicts[batch_id]
            final_true[start_idx:end_idx, :] = targets[batch_id]

        # 保存路径
        folder_path = './results/weather/rolling_predictions_per_channel_720/'
        ...
        for ch in range(num_channels):
            ...
            np.savez(os.path.join(folder_path, f'channel_{ch}.npz'), **channel_data)

            # plt.figure(figsize=(12, 4))
            # plt.plot(final_true[:, ch], label='True Values', color='blue', linewidth=2)
            # plt.plot(final_pred[:, ch], label='Predicted Values', color='red', linestyle='--', linewidth=2)
            # plt.title(f'Channel {ch} - Prediction vs True (Unique Time Steps)')
            # plt.xlabel('Time Step')
            # plt.ylabel('Value')
            # plt.legend()
            # plt.tight_layout()
            # plt.savefig(os.path.join(folder_path, f'channel_{ch}_unique.png'), dpi=300)
            # plt.close()
